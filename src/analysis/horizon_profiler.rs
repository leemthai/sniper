...
pub fn generate_profile(
    ...
    if total_candles == 0 { return profile; }
    ...
    if is_x86_feature_detected!("avx512f") {
        ...
        unsafe {
            ...
            for i in (0..main_loop_len).step_by(stride) {
                ...
                _mm256_storeu_si256(calculated_indices.as_mut_ptr().add(i) as *mut _, final_indices);
            }
        }
    }
    ...
    let scalar_logic = |i: usize, dest: &mut [u32]| {
        ...
        let dist_below = if h < current_price { (current_price - h) / current_price } else { 0.0 };
        let dist_above = if l > current_price { (l - current_price) / current_price } else { 0.0 };
    ...
    for i in 0..total_candles {
        ...
        if idx != u32::MAX {
            ...
            if start_index <= steps {
                ...
                if ts < min_ts_map[start_index] { min_ts_map[start_index] = ts; }
                if ts > max_ts_map[start_index] { max_ts_map[start_index] = ts; }
            }
        }
    }
    ...
    if DEBUG_FLAGS.enable_perf_logging {
        log::error!(
            "TRACE: HorizonProfiler [{}]: Total {:.2?} (Prep: {:.2?} | Math: {:.2?} | Agg: {:.2?})",
            pair, t_total, t_prep, t_math, t_agg
        );
    }
